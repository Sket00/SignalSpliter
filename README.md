...
FREE --> LOCAL_HELD : Local control
FREE --> REMOTE_HELD : Remote control

LOCAL_HELD --> FREE : Release
LOCAL_HELD --> FREE : 30 s timeout

REMOTE_HELD --> FREE : Release
REMOTE_HELD --> FREE : 30 s timeout

LOCAL_HELD --> LOCAL_HELD : Activity\n(reset timer)
REMOTE_HELD --> REMOTE_HELD : Activity\n(reset timer)

REMOTE_HELD --> STEAL_PROMPT : Local request
STEAL_PROMPT --> LOCAL_HELD : Steal lock
STEAL_PROMPT --> REMOTE_HELD : Cancel

REMOTE_HELD --> REMOTE_HELD : Warning
LOCAL_HELD --> LOCAL_HELD : Access Denied
...
class FREE free
class LOCAL_HELD local
class REMOTE_HELD remote
...
The SPI1-based fix was validated on a prototype using direct wiring to the Raspberry Pi's GPIO header. The currently designed PCB layout routes only the primary SPI bus (SPI0) to the display connector; the auxiliary bus (SPI1) is not currently broken out to that connector. Deploying this fix on the fabricated PCB as currently designed would require either a board revision routing SPI1 to the display connector, or a manual patch wire bridging the relevant GPIO18/20/21 pins directly from the Pi's header to the display connector.
